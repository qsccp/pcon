...
namespace ndn {
    ...
    ConsumerPconZipf::ScheduleNextPacket()
    {
        ...
        if (this->greedyFlag) {
            ...
            m_sendEvent = Simulator::Schedule(NanoSeconds(waitTime), &ConsumerPconZipf::SendPacket, this);
            }
            return;
        }
        ...
        if (this->m_firstTime) {
            ...
            if (this->timingStop > 0) {
                Simulator::Schedule(MilliSeconds(this->timingStop), &ConsumerPconZipf::stop, this);
            }

            // 如果开启了延迟贪婪，则在指定时间后客户端开始贪婪
            if (this->delayGreedy > 0 && this->greedyRate > 0) {
                Simulator::Schedule(MilliSeconds(this->delayGreedy), &ConsumerPconZipf::startGreedy, this);
            } else {
            }
            m_sendEvent = Simulator::Schedule(MilliSeconds(this->delayStart), &ConsumerPconZipf::SendPacket, this);
        } else {
